  ...
  if (!cart) {
    try {
      ...
      throw new ApiError(
        ...
        "User cart creation failed because user already have an cart"
      );
    }
  }
  if (cart == null) {
...
const updateProductInCart = async (user, productId, quantity) => {
  ...
  if (cart == null) {
    throw new ApiError(
      httpStatus.BAD_REQUEST,
      "User does not have a cart. Use POST to create cart and add a produc"
    );
  }
  let product = await Product.findById(productId);
  if (product == null) {
...
const deleteProductFromCart = async (user, productId) => {
  let cart = await Cart.findOne({ email: user.email });
  if (cart == null) {
    throw new ApiError(httpStatus.BAD_REQUEST, "User does not have an cart");
  }
